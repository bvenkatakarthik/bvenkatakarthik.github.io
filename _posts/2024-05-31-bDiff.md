...
[Integration of maps ${ f : [a, b] \to E }$](#4); [Mean value theorem](#5)  

###### <a name="4"></a> [Back to top](#top).

$${ \underline{\textbf{Integration of maps } f : [a, b] \to E} }$$ 

The goal is to study integration of maps ${ f : [a,b] \to E }$. 
...
We can ask ourselves: How can we characterise the regulated maps ${ f : [a, b] \to E }$? 
...
We can ask ourselves: How does the integral behave with the usual operations on functions? 
...
Let ${ f : [a, b] \to E }$ be a regulated map. We can ask ourselves: What is the derivative of the function ${ F(x) = \int _a ^x f }$? 
...
> Especially 
> 
> $${ \boxed{ \int _a ^b f = \hat{F}(b) - \hat{F}(a) } }$$ 
...
Integrating this gives the required identity. ${ \blacksquare }$
...
$${ \int _0 ^{\pi} A(t) \, dt = \int _0 ^{\pi} A _F (t) \, dt = \begin{pmatrix} 0 &\pi ^2 /2 \\ 2 &\pi ^3 /3  \end{pmatrix} . \, \, \blacksquare   }$$ 

**Def**: Let ${ \mathcal{P} = (\alpha _0, \ldots, \alpha _n) }$ be a partition of ${ [\alpha, \beta] . }$ Now 

$${ \Delta _{\mathcal{P}} = \max _{1 \leq j \leq n} (\alpha _j - \alpha _{j-1})  }$$ 

is called the mesh of the partition, and any element ${ \xi _j \in [\alpha _{j-1}, \alpha _j] }$ is called a between point. 

**Thm** [Regulated integral agrees with the Riemann integral]:    
Let ${ E }$ be a complete normed space, and ${ f : [\alpha, \beta] \to E }$ a regulated map.    
Then for every ${ \varepsilon > 0 }$ there is a ${ \delta > 0 }$ such that 

$${ \left\vert \int _{\alpha} ^{\beta}  f \, dx - \sum _{i=1} ^{n} f(\xi _j) (\alpha _j - \alpha _{j-1})   \right\vert < \varepsilon  }$$ 

for all partitions ${ \mathcal{P} = (\alpha _0, \ldots, \alpha _n) }$ of ${ [\alpha, \beta] }$ of mesh ${ \Delta _{\mathcal{P}} < \delta  }$ and all between points ${ \xi _j \in [\alpha _{j-1}, \alpha _j] . }$ 

**Pf**: **(i)** Suppose first ${ f : [\alpha, \beta] \to E }$ is a step map. More specifically ${ f }$ is a step map wrt the partition ${ \hat{\mathcal{P}} = (\hat{\alpha} _0, \ldots, \hat{\alpha} _{\hat{n}}) }$ and ${ e _j = f \vert (\hat{\alpha} _{j-1}, \hat{\alpha} _j) .   }$    
Let ${ \varepsilon > 0 . }$ It suffices to find a ${ \delta > 0 }$ such that all Riemann sums of mesh less than ${  \delta }$ are within ${ \varepsilon }$ of ${ \int _{\alpha} ^{\beta} f \, dx . }$    
Consider a partition ${ \mathcal{P} = (\alpha _0, \ldots, \alpha _n) }$ and between points ${ \xi _j \in [\alpha _{j-1}, \alpha _j]. }$    
Now for ${ (\beta _0, \ldots, \beta _m) }$ ${ = \mathcal{P} \cup \hat{\mathcal{P}} }$ we have 

$${ {\begin{aligned} &\, \int _{\alpha} ^{\beta} f \, dx - \sum _{j=1} ^{n} f(\xi _j) (\alpha _j - \alpha _{j-1}) \\ = &\, \sum _{i=1} ^{\hat{n}} e _i  (\hat{\alpha} _i - \hat{\alpha} _{i-1}) - \sum _{j=1} ^{n} f(\xi _j) (\alpha _j - \alpha _{j-1}) \\ = &\, \sum _{k=1} ^{m} e _k ^{’} (\beta _k - \beta _{k-1}) - \sum _{k=1} ^{m} e _k ^{’’} (\beta _k - \beta _{k-1})   \end{aligned}}   }$$ 

where 

$${ {\begin{aligned} &\, e _k ^{’} : = f \vert ( \beta _{k-1}, \beta _k ) , \, \text{ and } \\ &\, e _k ^{’’} := f(\xi _j)\text{ where } j \text{ is such that } [\beta _{k-1}, \beta _k] \subseteq [\alpha _{j-1}, \alpha _j] . \end{aligned}} }$$ 

Note that ${ e _k ^{’} = e _k ^{’’}   }$ except perhaps at partition points ${ \lbrace \hat{\alpha} _0, \ldots, \hat{\alpha} _{\hat{n}} \rbrace .  }$ Hence the above sum 

$${ \sum _{k=1} ^{m} (e _k ^{’} - e _k ^{’’}) (\beta _k - \beta _{k-1})   }$$ 

has atmost ${ 2 \hat{n} }$ nonzero terms. For each such term we have 

$${  \vert (e _k ^{’} - e _k ^{’’} )  (\beta _k - \beta _{k-1}) \vert \leq 2 \lVert f \rVert _{\infty} \Delta _{\mathcal{P}} ,   }$$ 

and hence 

$${ \left\vert \int _{\alpha} ^{\beta} f \, dx - \sum _{j=1} ^{n} f(\xi _j) (\alpha _j - \alpha _{j-1})  \right\vert \leq 4 \hat{n}  \lVert f \rVert _{\infty} \Delta _{\mathcal{P}} .  }$$ 

Choosing ${ \delta := \varepsilon / (4 \hat{n} \lVert f \rVert _{\infty}),  }$ we see 

$${ \Delta _{\mathcal{P}} < \delta \implies \left\vert \int _{\alpha} ^{\beta} f \, dx - \sum _{j=1} ^{n} f(\xi _j) (\alpha _j - \alpha _{j-1})  \right\vert < \varepsilon  }$$ 

as needed. 

**(ii)** Now say ${ f : [\alpha, \beta] \to E }$ is a regulated map. Let ${ \varepsilon > 0 . }$    
There is a step map ${ g : [\alpha, \beta] \to E }$ such that ${ \lVert f - g \rVert _{\infty} < \varepsilon . }$ From **(i)**, for every partition ${ \mathcal{P} = (\alpha _0, \ldots, \alpha _n) }$ with mesh ${ \Delta _{\mathcal{P}} < \delta }$ and ${ \xi _j \in [\alpha _{j-1}, \alpha _j] }$ we have 

$${ \left\vert \int _{\alpha} ^{\beta} g \, dx - \sum _{i=1} ^{n} g(\xi _j)  (\alpha _j - \alpha _{j-1})   \right\vert < \varepsilon .  }$$ 

Hence 

$${ {\begin{aligned} &\, \left\vert \int _{\alpha} ^{\beta} f \, dx - \sum _{i=1} ^{n} f(\xi _j) (\alpha _j - \alpha _{j-1})  \right\vert \\ \leq &\, \left\vert \int _{\alpha} ^{\beta} (f-g) \, dx \right\vert \\ + &\, \left\vert \int _{\alpha} ^{\beta} g \, dx - \sum _{i=1} ^{n} g(\xi _j)  (\alpha _j - \alpha _{j-1})   \right\vert \\ + &\, \left\vert \sum _{i=1} ^{n} (g(\xi _j) - f(\xi _j)) (\alpha _j - \alpha _{j-1})  \right\vert  \\ \leq &\, \varepsilon (\beta - \alpha) + \varepsilon + \varepsilon (\beta - \alpha)  \end{aligned}} }$$ 
...
The goal is to study mean value theorem for maps ${ f : U (\subseteq E \text{ open}) \to F . }$ 
