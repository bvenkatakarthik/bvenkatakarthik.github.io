...
Sections: Second derivative; Third derivative; Higher derivatives; Taylor’s theorem. 
...
> Here ${ \lambda }$ in ${ L(E, L(E, F)) }$ takes inputs as ${ \lambda (\textunderscore) (\textunderscore) ,}$ and ${ \Phi (\lambda) }$ in ${ L ^2 (E; F) }$ takes inputs as ${ \Phi (\lambda) (\textunderscore, \textunderscore) .}$    
> Writing (informally) ${ \Phi (\lambda) }$ as ${ \lambda (\textunderscore, \textunderscore) ,}$ the normed space isomorphism is 
> 
> $${ \begin{align*} &\, L(E, L(E, F)) \overset{\cong}{\longleftrightarrow} L ^2 (E; F) , \\ &\, \quad \lambda (\textunderscore) (\textunderscore) \quad \longleftrightarrow \quad \lambda (\textunderscore, \textunderscore) . \end{align*} }$$ 
...
Informally, defining ${ f _0 := f(\textunderscore) (\textunderscore) }$ works. Formally, consider 
