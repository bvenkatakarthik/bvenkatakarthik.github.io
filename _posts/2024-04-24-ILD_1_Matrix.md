...
Consider ${ A \in \mathbb{R} ^{M \times M} }$ with ${ A > 0 }$ (here this means it is symmetric positive definite).  We can partition it as 
...
Let ${ x, y \in \mathbb{R} ^N }$ be nonzero. We can try decomposing ${ x }$ as ${ x = x _{||} + x _{\perp} , }$ so that ${ x _{||} }$ is parallel to ${ y }$ and ${ x _{\perp} }$ perpendicular to ${ y .}$ This amounts to writing ${ x = ty + (x - ty) }$ where ${ \langle x - ty, y \rangle = 0 .}$ Here ${ \langle x, y \rangle }$ is the standard Euclidean form ${ \langle x, y \rangle = x ^T y .}$ So ${ x = \frac{\langle x, y \rangle}{\langle y, y \rangle} y + \left( x - \frac{\langle x, y \rangle}{\langle y, y \rangle} y \right) }$ is the required decomposition. We write ${ x _{||} =  \text{proj} _y (x) = \frac{\langle x, y \rangle}{\langle y, y \rangle} y .  }$ 

Consider linearly independent columns ${ A = (A _1, \ldots, A _M) \in \mathbb{R} ^{N \times M} }$ (so ${ M \leq N }$). A natural way to turn it to an orthonormal set ${ (Q _1, \ldots, Q _M) }$ is: 
...
Formally, we can inductively/successively verify the (ordered) sets 

$${ (P _1),  (P _1, P _2), \ldots, (P _1, \ldots, P _M) }$$ 

are orthogonal and with nonzero elements (hence ${ (Q _1), (Q _1, Q _2), \ldots, (Q _1, \ldots, Q _M) }$ are orthonormal). 
