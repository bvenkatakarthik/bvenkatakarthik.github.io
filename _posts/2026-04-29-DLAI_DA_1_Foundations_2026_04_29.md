...
Consider the hotel reservation dataset in Google Sheets. 

Suppose you want to order the rows by date. 

To sort by year: 

* Click on the filter option for arrival_year. 
...
* Check the box "Data has a header row". Set up Sort by arrival_year A to Z. 

* Click on add another sort column. Set up Sort by arrival_month A to Z. 

* Click on add another sort column. Set up Sort by arrival_date A to Z. 

<div align="center">
    <img src="https://c.l3n.co/cbUaWe.png"/> 
</div>

Note that the avg_price_per_room mentions price in euros. 

To format the column in euros: 

* Select the avg_price_per_room column 

* Select more formats (the "123" button) 

* Select Custom currency. Select Euro. 

[**Named Ranges**] 

A named range is essentially a label that you assign to a cell or a group of cells. Instead of referencing the cells by their standard notation (like A1:B10), you can use a descriptive name that reflects the purpose or content of the data. For example, you might name a range "SalesData" to represent a block of cells containing sales figures.

Creating a named range: 

Select the range. Go to Data -> Named ranges and define the name. 

Using a named range: 

For example, if you named a range "SalesData", you could use =SUM(SalesData) (enter this formula in a new cell) to calculate the sum of all cells within that range.

[**Getting to know your data**] 

**Techniques for getting to know your data** 
...
<div align="center">
    <img src="https://b.l3n.co/cbctii.png"/> 
</div>
...
Eg: Humans -> Survey -> Everyone may not respond. 

A system -> Look for systematic errors. 

* What is the quality of the data? 

Is it accurate, or are there imperfections? 

For eg, consider the specifications table in the Hotel bookings paper. 

<div align="center">
    <img src="https://b.l3n.co/cbc6lC.png"/> 
</div>
...
Consider a survey of active Netflix subscribers. 
...
Feature: Age group --> Summary statistic: Frequency of each group --> Insight: Type of content produced. 

Feature: Subscription duration --> Summary statistic: Range of subscription lengths --> Insight: Promotional rates?

(You'll want to understand the typical duration so you can figure out how to lengthen it.) 

You should also understand relationship between features. 

Eg: How does Subscription duration (feature) vary across Age group (feature)? 

**Relations to other data sets** 

Your dataset might also relate to other datasets, often connected by one or more common features. 

Eg:  A dataset of movies ​may relate to a dataset of users, ​linked based on movies watched by each user.

**Hotel reservations data set** 

Consider the no_of_adults feature.

We can create a new sheet and calculate the summary statistics for the no_of_adults feature. 

For min: 

Use `=min(`, select no_of_adults column in the original sheet, and then close the parenthesis `)`. 
...
Use `=average()` like above. 

Note that the min no_of_adults is 0. 

So.. how many children only bookings do we have? 

We can use "Sort from A to Z" on the no_of_adults column. 

We can see there are quite a few children only bookings... upto row 140. 

Similarly we can check the summary statistics of other features (if for a feature the mins or maxs are a bit absurd, we can sort and check how many such outliers are there in that feature.) 
